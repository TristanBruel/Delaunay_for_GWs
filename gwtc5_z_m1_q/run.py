...
def set_initial_states(
        branch_names, ndims, nleaves_max,
        ntemps, nwalkers, priors, log_like_fn,
        save_file=None, save=False):
    """
    Generate initial delaunay proposals
    """
    coords = {}
    inds = {}
    for branch in branch_names:
        ...
        #inds[branch][:, :, :] = True
    ## Initial states from random draws ##
    """
    ...
    """
    ## Initial states from final states of previous runs ##
    old_backend_file = '../lvk_z_m_q/outdir0/backend'
    with open(old_backend_file, "rb") as f:
        old_backend = pickle.load(f)
    old_chain = old_backend.get_chain()
    old_inds = old_backend.get_inds()
    for t, w in product(range(ntemps), range(nwalkers)):
        le_log = -np.inf
        for _ in trange(10_000):
            ind_step = np.random.randint(0,old_chain['tri'].shape[0])
            ind_walker = np.random.randint(0,old_backend.nwalkers)
            ind_temp = np.random.randint(0,old_backend.ntemps)
            init_proposal = {
                    'tri':
                    old_chain['tri'][ind_step, ind_temp, ind_walker][old_inds['tri'][ind_step, ind_temp, ind_walker]]
            } | {
                    branch:
                    old_chain[branch][ind_step, ind_temp, ind_walker, 0]
                    for branch in old_backend.branch_names
                    if branch != 'tri'
            }
            init_proposal['cos_tilt'] = init_proposal.pop('tilt')
            le_log = (
                log_like_fn(
                    [
                        init_proposal[key]
        ...
        for branch in init_proposal:
            coords[branch][
                t, w, : (init_proposal[branch].shape[0] if branch == "tri" 
                    else nleaves_max[branch])
            ] = init_proposal[branch]
            inds[branch][t, w, : (init_proposal[branch].shape[0] if branch == "tri"
                                        else nleaves_max[branch])
            ] = True
    if save:
