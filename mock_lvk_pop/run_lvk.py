...
detected_injections[:,2] /= detected_injections[:,0] # turn secondary mass into mass ratio
injection_priors = injections_file["priors_inj"]
num_injections = injections_file["ninj"]
...
class M1ZQDelaunay:
    ...
    def __call__(self, population_parameters):
        ...
        (inner_tri_parameters, corner_weights) = population_parameters
        ...
        log_dNdtheta = log_dNdtheta_tri.reshape(self.num_events, self.num_samples)
...
branch_names = ["tri", "corner_w"]
ndims = dict(zip(branch_names, [4, 8]))
...
barycenters = observed_events.reshape(num_events, num_samples, 3).mean(axis=1)
